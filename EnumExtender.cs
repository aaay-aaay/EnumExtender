...
namespace PastebinMachine.EnumExtender
{
	...
	public class EnumExtender : PartialityMod
	{
		...
		public override void OnLoad()
		{
			...
			foreach (Type type in list)
			{
				try
				{
					foreach (FieldInfo fieldInfo in type.GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
					{
						...
						{
							Debug.LogError(type.FullName + "." + fieldInfo.Name + " does not contain an enum type. It should be fixed, or moved out of the EnumExt_ class.");
							return;
						}
			...
			foreach (EnumValue enumValue in EnumExtender.declarations)
			{
				try
				{
					Type type2;
					EnumBuilder enumBuilder;
					if (!EnumExtender.enums.TryGetValue(enumValue.type, out type2))
					{
						enumBuilder = EnumExtender.module.DefineEnum("EnumExtender." + enumValue.type.Assembly.GetName().Name.Replace("-", "") + "." + enumValue.type.FullName.Replace("+", ""), TypeAttributes.Public, Enum.GetUnderlyingType(enumValue.type));
                        type2 = enumBuilder;
						EnumExtender.enums[enumValue.type] = enumBuilder;
					...
					list2.Add(new KeyValuePair<IReceiveEnumValue, object>(enumValue.receiver, Enum.ToObject(enumValue.type, obj3)));
				}
				catch (Exception ex)
				{
